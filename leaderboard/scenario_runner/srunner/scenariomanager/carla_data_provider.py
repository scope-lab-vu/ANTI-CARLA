...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_batch_actors(model, amount, spawn_points, autopilot=False,
        ...
        #waypoints = CarlaDataProvider._world.get_map().generate_waypoints(10)
        #batch = []
        for i in range(amount):
            ...
            if random_location:
                ...
                if count < 1:
                    #for w in waypoint.next_until_lane_end(10):
                    w = waypoint.next_until_lane_end(20)[0]
                    if w.transform not in appeared:
                        #appeared.append(w.transform.location)
                        appeared.append(w.transform)
