...
class StatisticsManager(object):
    ...
    def save_global_record(route_record,infractions_to_record, sensors, total_routes, endpoint,filename,stats_file):
        ...
        infractions_red = float(stats_dict['infractions']['red_light'])
        infractions_stop = float(stats_dict['infractions']['stop_infraction'])
        out_of_lane = stats_dict['infractions']['outside_route_lanes']
        route_dev = stats_dict['infractions']['route_dev']
        driving_score = round(stats_dict['scores']['score_composed'],2)
        timeout = stats_dict['infractions']['route_timeout']

        myvalues = [{'driving score': '{:.3f}'.format(stats_dict['scores']['score_composed']),
                    'collisions':'{:.3f}'.format(stats_dict['infractions']['collisions_vehicle']), 'infractions':'{:.3f}'.format(stats_dict['infractions']['red_light']),
                    'out of lane':'{:.3f}'.format(stats_dict['infractions']['outside_route_lanes']), 'timeout':'{:.3f}'.format(stats_dict['infractions']['route_timeout'])}]
        ...
        stats.append(infractions_red)
        stats.append(infractions_stop)
        stats.append(out_of_lane)
        stats.append(route_dev)
        stats.append(timeout)

        # myvalues1 = [{'driving score': driving_score,
        #             'collisions':collision_lane, 'infractions':infractions,
        #             'out of lane':out_of_lane, 'timeout':timeout}]

        # with open(stats_file, 'a') as csvfile1:
        #     writer = csv.DictWriter(csvfile1, delimiter = ',',fieldnames = data['labels'])
        #     if not file_exists:
        #         writer.writeheader()
        #     writer.writerows(myvalues1)
        ...
        return collision_lane, infractions_red, out_of_lane, driving_score
